export const BASE_URL = 'https://project-planner-tsk.herokuapp.com/';
export const WEB_URL = 'https://project-planner-tsk.netlify.app/';

export const USERS_URL = 'https://project-planner-tsk.herokuapp.com/users';
//export const USERS_URL = "http://localhost:8080/users";

export const PROJECTS_URL =
  'https://project-planner-tsk.herokuapp.com/projects';
//export const PROJECTS_URL = "http://localhost:8080/projects";

export const SESSIONS_URL =
  'https://project-planner-tsk.herokuapp.com/sessions';
//export const SESSIONS_URL = "http://localhost:8080/sessions";

export const INVITE_URL =
  'https://project-planner-tsk.herokuapp.com/inviteUser';
//export const INVITE_URL = "http://localhost:8080/inviteUser";

export const COMMENTS_URL =
  'https://project-planner-tsk.herokuapp.com/comments';
//export const COMMENTS_URL = "http://localhost:8080/comments";
